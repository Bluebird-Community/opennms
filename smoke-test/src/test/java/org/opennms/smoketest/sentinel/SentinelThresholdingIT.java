...
public class SentinelThresholdingIT {
        ...
        public static final OpenNMSStack stack = OpenNMSStack.withModel(StackModel.newBuilder()
                        .withOpenNMS(OpenNMSProfile.newBuilder()
                                        .withFile("sentinel-thresholding/threshd-configuration.xml",
        ...
        public void canTriggerHighThresholdWithJti() throws InterruptedException, IOException {
                ...
                final InetSocketAddress minionListenerAddress = stack.minion()
                                .getNetworkProtocolAddress(NetworkProtocol.JTI);
                ...
                await().atMost(Duration.ofMinutes(1)).pollInterval(Duration.ofSeconds(5))
                                .until(() -> getAlarmsForNode(onmsNode.getId())
                                                .contains(new AbstractMap.SimpleImmutableEntry<>(
        ...
        private AbstractAdapterIT.RequisitionCreateInfo getRequisitionToCreate() {
                final AbstractAdapterIT.RequisitionCreateInfo requisitionCreateInfo = new AbstractAdapterIT.RequisitionCreateInfo();
        ...
        private void sendTriggerHighThresholdMessages(InetSocketAddress minionListenerAddress)
                ...
                // We need the delta sum of in+out to exceed 100,000/s keeping in mind we slept
                // for 30 seconds
                new Packet(buildJtiMessage(NODE_IP, INTERFACE_ID, 10000000, 10000000, 1))
                                .send(Sender.udp(minionListenerAddress));
        }

        private void sendClearHighThresholdMessages(InetSocketAddress minionListenerAddress)
        ...
        private void sleepToEnsureSeparateDelivery() throws InterruptedException {
                // We sleep here due to current limitations in how received packets are
                // timestamped. The timestamp is derived
                // from the received time so we need to ensure they arrive separately at the
                // Sentinel in spite of any collating
                // that may be attempted on the path from Minion to Sentinel which may require a
