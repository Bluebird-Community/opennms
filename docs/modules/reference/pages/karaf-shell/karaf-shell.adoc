...
User authentication is performed with the same back-end as the WebUI, however, access to the Karaf shell is limited to users with xref:operation:deep-dive/user-management/security-roles.adoc#built-in-security-roles[ROLE_ADMIN].
...
[[karaf-shell-ssh-key]]
=== Authenticating with an SSH public key
By default, the shell authenticates with a username and password against the same back end as the web UI.
As an alternative, a user can authenticate with an SSH public key.
Password authentication remains available: a key is tried first, and a connection that presents no matching key falls back to a password prompt.

To allow a user to authenticate with a key, add the public key to `etc/keys.properties`, one entry per line, in the form `username = <key>,<group>`:

[source, properties]
----
myuser = AAAAB3NzaC1yc2EAAA...,_g_:admingroup
----

Include only the base64 key material, without the leading key-type prefix (for example, `ssh-rsa`) and without any trailing comment.
The `_g_:admingroup` reference assigns the user to the `admingroup` group defined elsewhere in the same file; that group grants the roles associated with the key.
Because shell access is limited to users with xref:operation:deep-dive/user-management/security-roles.adoc#built-in-security-roles[ROLE_ADMIN], the group must grant that role.

Connect using the matching private key:

[source, console]
----
ssh -i ~/.ssh/id_rsa -p 8101 myuser@localhost
----

`etc/keys.properties` is read on each login attempt, so changes apply to subsequent connections without restarting {page-component-title}.
...
|Enabled By Default
|opennms:add-detector-to-foreignsource
|Adds a detector to a foreign source definition, creating the foreign source definition if it does not exist
|Enabled By Default
|opennms:add-requisitioned-node
|Adds a node to a requisition, creating the requisition if it does not exist
|Enabled By Default
|opennms:alarm-clear
|Clears current alarms meeting certain criteria
|Enabled By Default
|opennms:alarm-list
...
|Enabled By Default
|opennms:delete-detector-from-foreignsource
|Deletes a detector from a foreign source definition
|Enabled By Default
|opennms:delete-foreignsource
|Delete a named foreign source definition
|Enabled By Default
|opennms:delete-measurement-resource
|Delete the measurements and meta-data for a given resource ID
|Enabled By Default
|opennms:delete-requisition
|Delete a named Provisioning Requisition
|Enabled By Default
|opennms:delete-requisitioned-node
|Delete a node from a Provisioning Requisition
|Enabled By Default
...
|opennms:ha-config
|Display or modify the high-availability configuration for this instance
|xref:operation:deep-dive/admin/high-availability.adoc[opennms-ha-shell]
|opennms:ha-failover
|Step this ACTIVE instance down so the partner can promote
|opennms-ha-shell
|opennms:ha-status
|Show the state, heartbeat age, and roles of both HA instances
|opennms-ha-shell
endif::[]
...
|xref:operation:deep-dive/kafka-producer/kafka-producer.adoc[opennms-kafka-producer]
...
|Show the Kafka topic that metrics for the given node, interface or service are routed to
|opennms-kafka-producer
...
|Enabled By Default
|opennms:scv-get-all
|Retrieves the credentials for all aliases
|Enabled By Default
|opennms:scv-list
...
|Enabled By Default
|opennms:show-foreignsource-definition
|Display defined Foreign Source Definitions
|Enabled By Default
|opennms:show-import
...
|Enabled By Default
|opennms:show-requisition
|Display the content of a named Provisioning Requisition
|Enabled By Default
|opennms:snmp-fit
...
|Enabled By Default
|opennms:synchronize-requisition
|Sends an `uei.opennms.org/internal/importer/reloadImport` event to import the requisition from a given url parameter
|Enabled By Default
