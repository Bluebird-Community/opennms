...
public class OpenNMSSeleniumIT extends AbstractOpenNMSSeleniumHelper {
     ...
     * The default, RECORD_FAILING will only save recordings when there are failed
     * tests.
     * Other options: SKIP or RECORD_ALL.
     *
     * Historical warning: This container can fail and cause test instability - for
     * example if retrieving the
     * recording from the container fails, an exception will be thrown causing the
     * test to fail.
     *
     * Although RECORD_FAILING is the default in BrowserWebDriverContainer, the
     * constructor we use resets
     * it to SKIP. :( So we always explicitly set it.
     */
    private static final BrowserWebDriverContainer.VncRecordingMode RECORDING_MODE = BrowserWebDriverContainer.VncRecordingMode.RECORD_FAILING; // RECORD_ALL,
                                                                                                                                                // RECORD_FAILING,
                                                                                                                                                // or
                                                                                                                                                // SKIP

    // Use mp4 since everything supports it
    private static final VncRecordingContainer.VncRecordingFormat RECORDING_FORMAT = VncRecordingContainer.VncRecordingFormat.MP4;
    ...
    private static final WorkaroundBrowserWebDriverContainer firefox_instance = new WorkaroundBrowserWebDriverContainer()
            ...
            // Increase the containers shared memory to 2GB to help prevent Firefox from
            // crashing
    ...
    public static FirefoxOptions getFirefoxOptions() {
        ...
        options.addPreference("browser.download.folderList", 2); // Use for the default download directory the last
                                                                 // folder specified for a download
        options.addPreference("browser.download.dir", "/tmp/firefox-downloads"); // Set the last directory used for
                                                                                 // saving a file from the "What should
                                                                                 // (browser) do with this file?"
                                                                                 // dialog.
        options.addPreference("browser.helperApps.neverAsk.saveToDisk", "application/pdf"); // List of MIME types to
                                                                                            // save to disk without
                                                                                            // asking what to use to
                                                                                            // open the file
    ...
    /**
     * Store the latest failure exception here.
     */
    @RegisterExtension
    public final TestWatcher watchman = new TestWatcher() {
        @Override
        public void testFailed(ExtensionContext context, Throwable cause) {
            failed = Optional.of(cause);
        }
    };
    ...
    /**
     * We work around some bugs and shortcomings in org.testcontainers'
     * BrowserWebDriverContainer here until we can
     * submit issues/PRs upstream and the fixes are released.
     * First, when a test is complete, nothing is done to tell the VNC recorder
     * Docker image to stop recording.
     * Transcoding the recording is CPU-intensive and single-threaded and happens
     * while new data is coming in if the
     * VNC recorder process is still running. Unluckily for us, because of our large
     * screen size, the transcoder can't
     * keep up with the incoming data from the VNC recorder, so it never gets to EOF
     * on its input file and will run
     * forever. We work around that by sending a SIGSTOP to the VNC recorder process
     * inside its Docker container (we
     * can't just stop the container because we need it around and running because
     * that's where the transcoder runs).
     * Lastly, we provide an alternate RecordingFileFactory that caches the
     * recording file name and we output the
    ...
    public static class WorkaroundBrowserWebDriverContainer
        ...
        public void beforeEach(ExtensionContext context) {
            // These are static and reused across all test classes, so reset before every
            // test run
                ...
                if (vncRecordingContainer != null && vncRecordingContainer.isRunning()) {
                    ...
                    if (results.getExitCode() != 0) {
                        throw new RuntimeException(
                                "Got non-zero exit code " + results.getExitCode() + " when attempting to "
