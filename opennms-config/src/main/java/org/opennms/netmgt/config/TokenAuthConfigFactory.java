...
public final class TokenAuthConfigFactory {
    ...
    public static synchronized void reload() throws IOException {
        try (InputStream is = new FileInputStream(
                ConfigFileConstants.getFile(ConfigFileConstants.TOKEN_AUTH_CONFIG_FILE_NAME))) {
            final TokenAuthConfigFactory next = new TokenAuthConfigFactory(is);
            setInstance(next);
        }
    }
     ...
     * whose {@link TokenAuthConfiguration} was mutated post-construction
    ...
    static void validate(final TokenAuth auth) {
        ...
        if (auth.getBasicAuth() != null) {
            final org.opennms.netmgt.config.tokenauth.BasicAuth ba = auth.getBasicAuth();
