...
If a Minion loses connectivity with the {page-component-title} core instance (over gRPC or Kafka), then any received messages (syslog, flows, SNMP traps) are queued until connectivity is restored.
