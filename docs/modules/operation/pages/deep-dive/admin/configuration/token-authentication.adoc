...
* A downstream collector reports a `FAILED` collection status while a cached token was in use.
...
NOTE: A FAILED status that surfaces for non-auth reasons -- RPC timeout, broker outage, generic transport error -- also invalidates the cached token, since the adaptor sees only the result status and not the underlying cause.
The effect is benign: the next cycle re-acquires and proceeds.
A run of transient failures may add one extra token-acquisition round-trip per cycle until the transport recovers.
