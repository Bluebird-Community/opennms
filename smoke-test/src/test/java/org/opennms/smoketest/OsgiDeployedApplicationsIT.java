...
/**
 * Tests that sites deployed in the osgi container can be reached from jetty
 * (see NMS-7785).
 *
 * In addition it verifies that access to /osgi/** is not allowed (see
 * NMS-8431).
 *
 * @author mvrueden
 */
public class OsgiDeployedApplicationsIT extends OpenNMSSeleniumIT {
    ...
    private void verifyUrls(final String[] paths, final int expectedStatus, final String expectedStatusText)
            throws IOException {
        for (final String eachPath : paths) {
            ...
            connection.setRequestProperty("Authorization", "Basic " + Base64.getEncoder()
                    .encodeToString((OpenNMSContainer.ADMIN_USER + ":" + OpenNMSContainer.ADMIN_PASSWORD).getBytes()));
            connection.setConnectTimeout(250);
            connection.setUseCaches(false);
            connection.setDoOutput(false);
            connection.setInstanceFollowRedirects(false); // we do not want to follow redirects, otherwise 200 OK might
                                                          // be returned
            connection.connect();
            LOG.info("Status: {} {}", connection.getResponseCode(), connection.getResponseMessage());

            // All unauthorized requests are forwarded to login.jsp which will result in 200
            // OK, we
            ...
            if (302 == connection.getResponseCode()) {
                ...
                Assertions.fail("Request was forwarded. This is not allowed.");
            }

            // Valid request, now check for OK
            final String errorMessage = String.format("URL: %s: status %s (%s) expected but %s (%s) received.",
                    urlString, expectedStatus, expectedStatusText, connection.getResponseCode(),
                    connection.getResponseMessage());
            Assertions.assertEquals(expectedStatus, connection.getResponseCode(), errorMessage);
