...
/**
 * Verifies that we can issue scans on the Minion and generate newSuspect
 * events.
 *
 * @author jwhite
 */
...
public class DiscoveryIT {
        ...
        public static final OpenNMSStack stack = OpenNMSStack.withModel(StackModel.newBuilder()
                        .withOpenNMS(OpenNMSProfile.newBuilder()
                                        .withFile("empty-discovery-configuration.xml",
        ...
        public void canDiscoverRemoteNodes() throws IOException, InterruptedException {
                ...
                final HttpHost opennmsHttpHost = new HttpHost(stack.opennms().getContainerIpAddress(),
                ...
                executor.execute(Request.Post(
                                String.format("http://%s:%d/opennms/admin/discovery/actionDiscovery?action=AddSpecific",
                ...
                executor.execute(Request.Post(String.format(
                                "http://%s:%d/opennms/admin/discovery/actionDiscovery?action=SaveAndRestart",
                                opennmsHttpHost.getHostName(), opennmsHttpHost.getPort()))
                                .bodyForm(Form.form()
                ...
                await().atMost(Duration.ofMinutes(1)).pollInterval(Duration.ofSeconds(10))
