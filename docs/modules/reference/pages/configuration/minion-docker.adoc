...
.Settings when using gRPC

If you want to use gRPC, the environment variable names are converted with a prefix convention:

* Prefix `GRPC_IPC_` will be written to `org.opennms.core.ipc.grpc.client.cfg`

* Everything behind will be converted to lower case and `_` is replaced with `.`

Setting `GRPC_IPC_HOST` also enables the `opennms-core-ipc-grpc-client` feature at boot.
...
----
environment:
  - GRPC_IPC_HOST=192.168.1.1
  - GRPC_IPC_PORT=8990
----

This creates the file `org.opennms.core.ipc.grpc.client.cfg` with the content:
...
Setting `KAFKA_IPC_BOOTSTRAP_SERVERS` also enables the `opennms-core-ipc-kafka` feature at boot.
