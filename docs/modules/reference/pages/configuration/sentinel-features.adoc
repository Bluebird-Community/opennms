= Sentinel Features
:description: Discover some of the Sentinel features you can install manually in OpenNMS {page-component-title}: core, Kafka, flows, and so on.
...
[source, console]
----
echo "sentinel-kafka" | sudo tee -a \${SENTINEL_HOME}/etc/featuresBoot.d/features.boot <1>
echo "sentinel-flows" | sudo tee -a ${SENTINEL_HOME}/etc/featuresBoot.d/sentinel-features.boot <2>
----
<1> Install and start Kafka communication feature.
<2> Install and start Sentinel flows feature.

== Auto configure flow processing for Sentinel

The following example illustrates a `features.xml` which configures the _Sentinel_ instance and automatically starts
all required features to consume messages via _Kafka_.
...
In addition `sentinel-kafka` should be installed and configured properly.
...
In addition `sentinel-kafka` should be installed and be configured properly.
See the previous _Flow Processing_ chapter for more details.
...
==== User/password
Supplying the username/password via YAML file for configuration via confd is not supported.
