...
describe('NodeActionsDropdown.vue', () => {
  ...
  it('puts Info... first and includes all link actions', () => {
    ...
    expect(items).toHaveLength(13) // Info + 12 links as admin; Update SNMP needs an address
  })
  ...
  it('offers Site Status for a node with a building', () => {
    ...
    expect(items).toHaveLength(14) // Info + 13 links as admin; Update SNMP needs an address
  })
  ...
  describe('admin-only actions', () => {
    ...
    it('leaves a plain user the non-admin actions and Info...', () => {
      ...
      expect(labels).toHaveLength(9) // Info + 8 links; the 5 admin ones are gone
    })
