...
| true
| Enable or disable rescans of existing nodes following synchronization of a requisition
...
| `org.opennms.security.auditLog.apiAuth`
| false
| When enabled, REST API authentication successes are included in the `web-audit.log` audit log file.
  By default, only interactive web UI logins and all authentication failures are logged.
