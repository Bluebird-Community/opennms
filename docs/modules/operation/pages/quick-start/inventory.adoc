...
The default configuration also enables self-monitoring of the OpenNMS service, and discovers a single node with the interface `127.0.0.1` (see xref:deployment:core/install.adoc#first-monitored-node[First monitored node]).
...
* Specify more complex entity detection with OpenNMS detectors (DNS, FTP, JDBC, TCP, and so on).
