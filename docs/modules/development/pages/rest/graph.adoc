...
| /graphs/search/results/\{namespace}?providerId=\{providerid}&criteria=\{criteria}
| GET
| Resolves a suggestion to actual vertices.
The value for `\{providerid}` is defined in the suggestion.
...
[source, console]
----
curl -X GET -u admin:admin http://localhost:8980/opennms/api/v2/graphs/search/results/layer3-network?providerId=NodeSearchProvider&criteria=90
----
