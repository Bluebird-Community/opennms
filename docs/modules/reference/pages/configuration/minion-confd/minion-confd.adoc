...
It is not possible to supply the http username or password via the `minion-config.yaml` file.
...
When using Kafka instead of gRPC, settings for Kafka IPC (Twin) will take precedence over any RPC- and Sink-specific settings.
