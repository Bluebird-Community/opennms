...
public class GrafanaEndpointRestIT extends AbstractRestIT {
        ...
        public void verifyCRUD() {
                ...
                given().contentType(ContentType.JSON).body(endpoint).put(endpoint.getId().toString())
                                .then().log().all()
                                .assertThat().statusCode(202);
                final GrafanaEndpoint updatedEndpoint = given().get(endpoint.getId().toString()).then().statusCode(200)
                                .extract().as(GrafanaEndpoint.class);
        ...
        public void verifyValidation() {
                ...
                given().contentType(ContentType.JSON).body(endpoint).post()
                                ...
                                .assertThat().statusCode(400)
                                .body("context", is("url"))
                                .body("message", allOf(containsString("The provided URL"),
                                                containsString("not valid")));
        }
        ...
        public void verifyUidIsUnique() {
                ...
                given().contentType(ContentType.JSON).body(endpoint).post()
                                ...
                                .assertThat().statusCode(400)
                                .body("context", is("uid"))
                                .body("message", is(String.format("An endpoint with uid '%s' already exists.",
                                                endpoint.getUid())));
        }
