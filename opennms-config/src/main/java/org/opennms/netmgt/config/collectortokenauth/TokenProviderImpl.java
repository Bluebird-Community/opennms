 ...
 * {@link CollectorTokenAuthConfigFactory} (for definitions) and the {@link TokenCache}
 * (for current values).
 *
...
public class TokenProviderImpl implements TokenProvider {
    ...
    private final Supplier<CollectorTokenAuthConfigFactory> configSupplier;
     ...
     * {@code CollectorTokenAuthConfigFactory.init()} has been called.
     */
    public TokenProviderImpl(final Supplier<CollectorTokenAuthConfigFactory> configSupplier,
     ...
     * {@link CollectorTokenAuthConfigFactory#init()} lazily on first lookup. If
    ...
    private static CollectorTokenAuthConfigFactory lenientFactoryLookup() {
        try {
            CollectorTokenAuthConfigFactory.init();
            return CollectorTokenAuthConfigFactory.getInstance();
            ...
            LOG.debug("CollectorTokenAuthConfigFactory not yet initialised; ${token:...} will resolve as empty until init() completes", e);
            return null;
        }
    }
    ...
    public Optional<String> getToken(final String authName, final Scope callingScope) {
        ...
        final CollectorTokenAuthConfigFactory factory = configSupplier.get();
        if (factory == null) {
            return Optional.empty();
        }
