...
<8> Publish ports for Syslog, SNMP trap listener, and the SSH access to the xref:reference:karaf-shell/karaf-shell.adoc[Karaf shell].
...
<1> Replace `core-instance-ip` with the host of the gRPC endpoint on your {page-component-title} Core instance.
<2> Optional. You can set a unique human-readable Minion identifier; if not set, a unique identifier is generated.
