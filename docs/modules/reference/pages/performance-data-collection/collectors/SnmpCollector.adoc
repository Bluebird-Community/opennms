...
| Configuration       | Managed in the database via *Admin -> Manage SNMP Data Collection Config*. +
                        Pre-36 installations are migrated from `etc/datacollection-config.xml` and `etc/datacollection/*.xml` once at first boot; the original files are then archived to `etc_archive/`.
|===
...
Configure which MIB objects to collect from devices through *Admin -> Manage SNMP Data Collection Config*, or through the REST API under `/api/v2/datacollectionconf`.
The runtime reloads automatically when you save changes through the admin page; no service restart is required.

The XML structures shown below describe the schema imported on first-boot migration and accepted by the upload endpoint, and are useful when bulk-editing or version-controlling SNMP collection definitions outside the UI.

Understanding xref:operation:deep-dive/performance-data-collection/resource-types.adoc#resource-types[resource types] helps when authoring SNMP collection definitions.

.Structure of the imported `datacollection-config.xml` file
[source, xml]
----
<?xml version="1.0"?>
