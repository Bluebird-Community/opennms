...
This page collects worked examples showing how to configure the xref:performance-data-collection/collectors/xml.adoc[XmlCollector] against common HTTP APIs.
Each example follows the same shape so you can use one as a template for an integration of your own:

. *Endpoint(s) and response* -- the URLs being polled and the shape of what they return.
...
After reloading collectd (`send-event.pl uei.opennms.org/internal/reloadDaemonConfig --parm 'daemonName Collectd'`), test a one-shot collection from the xref:reference:karaf-shell/karaf-shell.adoc[Karaf shell] without waiting for the next scheduled cycle:
