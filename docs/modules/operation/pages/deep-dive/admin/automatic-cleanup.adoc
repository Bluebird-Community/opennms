...
| PostgreSQL autovacuum | Reclaiming dead tuples and updating statistics at the database level | `postgresql.conf` (This is part of Postgres and not part of {page-component-title})
|===

NOTE: The PostgreSQL autovacuum is a database-level concern. It reclaims space inside tables and indexes after rows are deleted, but it does not delete application rows itself. {page-component-title}'s mechanisms decide _which_ rows go; PostgreSQL decides when to compact what's left independently.
...
The PostgreSQL autovacuum daemon is separate from Vacuumd despite the similar name.
Autovacuum reclaims space and updates planner statistics after rows have been deleted; it does not delete application rows.

For most {page-component-title} workloads the default autovacuum settings are fine.
On high-volume systems with many events per second or high alarm volume, you may need to tune autovacuum on the busiest tables (`events`, `alarms`, `outages`) so dead-tuple cleanup keeps up with deletes.
That is a PostgreSQL operations topic; OpenNMS does not configure autovacuum on your behalf.
