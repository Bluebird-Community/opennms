...
public class JaegerTracingIT {
    ...
    public void minionEcho() throws Exception {
        await("Wait for a 'Echo' trace with two spans")
                ...
                .pollDelay(Duration.ZERO)
                .ignoreException(AssertionError.class)
                .untilAsserted(() -> givenWithFailureDetails((spec) -> {
                    spec
    ...
    /**
     * Wrap REST Assured calls to include additional failure details when there is
     * an AssertionError.
     *
     * @param consumer Consumer callback that will be passed the output of given()
     *                 with our filter() applied to capture
     *                 the response.
     *
     *                 If the callback throws an AssertionError, a new
     *                 AssertionError will be generated that includes the response
