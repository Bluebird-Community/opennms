...
The Kafka Producer additionally uses metadata directly, rather than through the DSL, to decide which topic to publish collected metrics to.
Define `metricRouting.package` in the `requisition` context on a node, interface, or service, and its metrics are published to the topic of that name -- see <<deep-dive/kafka-producer/configure-kafka.adoc#metric-routing, Route metrics to multiple topics>>.
The value is used verbatim as the topic name and is not interpolated.
...
* xref:operation:deep-dive/notifications/introduction.adoc[Notification configuration]: javamail-configuration.xml, javamail-configuration.properties
+
The outbound mail credential fields (`user-auth` on a `sendmail-config` in the XML, `authenticateUser`/`authenticatePassword` in the properties file) additionally resolve the `token` context (`${token:<name>}`), which supplies an access token from a xref:operation:deep-dive/admin/configuration/token-authentication.adoc[token authentication] definition.
This is mainly useful for OAuth2 (XOAUTH2) SMTP authentication.
